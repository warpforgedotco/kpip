...
from kpip_test_support.wheel import WheelBuilder, make_wheel
...
def test_install_from_future_wheel_version(
    ...
    """
    from kpip_test_support import TestFailure
    ...
    result = script.kpip("install", package, "--no-index", expect_error=True)
    ...
    result = script.kpip("install", package, "--no-index", expect_stderr=True)
    result.assert_installed("futurewheel", editable=False)
...
def test_basic_install_from_wheel_file(
    ...
    result = script.kpip("install", package, "--no-index")
    dist_info_folder = script.site_packages / "simple.dist-0.1.dist-info"
    result.did_create(dist_info_folder)
    ...
    with open(script.base_path / installer, "rb") as installer_file:
        ...
        assert installer_details == b"kpip\n"
    installer_temp = dist_info_folder / "INSTALLER.kpip"
...
def test_install_from_wheel_with_headers(script: KpipTestEnvironment) -> None:
...
def test_install_wheel_with_target(
    ...
    """Test installing a wheel using kpip install --target"""
    shutil.copy(shared_data.packages / "simple.dist-0.1-py2.py3-none-any.whl", tmpdir)
...
def test_install_wheel_with_target_and_data_files(
    ...
    result = script.kpip("install", package, "-t", target_dir, "--no-index")
...
def test_install_wheel_with_root(
    ...
    """Test installing a wheel using kpip install --root"""
...
def test_install_wheel_with_prefix(
    ...
    """Test installing a wheel using kpip install --prefix"""
...
def test_wheel_record_lines_in_deterministic_order(
    ...
    result = script.kpip("install", to_install)
...
def test_wheel_record_lines_have_updated_hash_for_scripts(
    ...
    """Kpip rewrites "#!python" shebang lines in scripts when it installs them;
...
def test_install_from_wheel_gen_unicode_entrypoint(script: KpipTestEnvironment) -> None:
    make_wheel(
...
def test_wheel_compile_syntax_error(
    ...
    result = script.kpip("install", "--compile", package, "--no-index")
...
def test_wheel_install_with_no_cache_dir(
    ...
    package = data.packages.joinpath("simple.dist-0.1-py2.py3-none-any.whl")
    result = script.kpip("install", "--no-cache-dir", "--no-index", package)
    result.assert_installed("simpledist", editable=False)


def test_wheel_install_fails_with_extra_dist_info(script: KpipTestEnvironment) -> None:
...
def test_wheel_installs_ok_with_nested_dist_info(script: KpipTestEnvironment) -> None:
    package = create_basic_wheel_for_package(
        script,
        "simple",
        "0.1.0",
    ...
    script.kpip("install", "--no-cache-dir", "--no-index", package)
...
def test_wheel_with_file_in_data_dir_has_reasonable_error(
    ...
    result = script.kpip("install", "--no-index", str(wheel_path), expect_error=True)
