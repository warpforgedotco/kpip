...
| `index/catalog_cache.py` | entries in `http-v3/` | parsed Simple API catalogs, release summaries (`Version.to_wire()`), target choices; key prefixes and payload headers carry their own versions; checksum-validated, recompiled from the catalog on any failure. A summary also records its page's freshness, updated in place on revalidation, so a resolve whose pages are fresh reads one file per project. `lock` also resolves from stale summaries while their pages revalidate in the background (`SimpleIndexSource.stale_summary`), keeping the result only if every one comes back 304 |
